  ...
  const setSliderBehavor = (type) => {
    if (sliderType === "NotYet") {
      ...
      if (type === "Right") {
        ...
        const tempArray = Object.values(slideArray);
        for (let i = 0; i < 4; i += 1) {
          tempArray.push(tempArray.shift());
        }
        // 0.75초 뒤에 SlideArray State에 저장
        setTimeout(() => {
          setSlideArray(tempArray);
        }, 750);
      } else {
        setSliderType("Left");
        const tempArray = Object.values(slideArray);
...
const SliderContent = styled.div`
  ...
  /* z-index: 0 ; */
  transform: ${(props) =>
    props.slideBehavior === "Base" ? "translate3d(-100%, 0px, 0px);" : ""};
  transform: ${(props) =>
    props.slideBehavior === "Right" ? "translate3d(-100%, 0px, 0px);" : ""};
  transform: ${(props) =>
    props.slideBehavior === "DoubleRight"
      ? "translate3d(-200%, 0px, 0px);"
      : ""};

  transition: transform 0.75s;
  transition: ${(props) =>
    props.slideBehavior === "Base" ? "transform 0s" : ""};
`;
